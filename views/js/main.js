...
var resizePizzas = function (size) {
    ...
    function changePizzaSizes(size) {
        ...
        // get for counter
        var pizzaCount = pizzacontainers.length;

        // calc new size change is the same for all
        var dx = determineDx(pizzacontainers[0], size);
        var newwidth = (pizzacontainers[0].offsetWidth + dx) + 'px';

        // set changed size to all
...
function updatePositions() {
    ...
    // get only the ones in this class
    var items = document.getElementsByClassName('mover');

    // get top distance retrieval out of loop
    var top = document.body.scrollTop / 1250;

    // do calculations out of loop (modulo % so only five different)
    var phases = [
        Math.sin(top + 0),
        Math.sin(top + 1),
        Math.sin(top + 2),
    ...
    // calculate and set  positions based on array
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // get pizza storage element before looping
    var movingPizzaList = document.getElementById("movingPizzas1")

    // reduced to 50 pizzas 
    for (var i = 0; i < 50; i++) {
        ...
        // append to retrieved object
        movingPizzaList.appendChild(elem);
    }
    updatePositions();
});
